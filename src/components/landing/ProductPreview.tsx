...
/** Намунаи дафтар — ҳар кор як слайд. */
export function ProductPreview() {
  ...
  return (
    <div className="w-full max-w-full overflow-hidden rounded-[1.4rem] border border-white/12 bg-[#151b26] shadow-[var(--shadow-xl)]">
      ...
      <div className="space-y-3 bg-[#f6f4ef] p-4">
        <WorkSlider
          compact
          label={t("notebookAria")}
          prevLabel={tl("prevSlide")}
          nextLabel={tl("nextSlide")}
          formatStatus={(current, total) => tl("slideStatus", { current, total })}
          slides={tiles.map((tile) => ({
            id: tile.n,
            node: (
              <div
                data-slide-body
                className="rounded-2xl border border-[#e2ddd4] bg-[#fffcf7] px-4 py-5"
              >
                <p className="font-mono text-[11px] text-primary">{tile.n}</p>
                <p className="mt-2 text-base font-semibold text-ink">{tile.title}</p>
                <p className="mt-1.5 text-sm leading-relaxed text-muted-foreground">{tile.text}</p>
              </div>
            ),
          }))}
        />
